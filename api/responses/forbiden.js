 ...
 *     return res.forbiden();
 *     // -or-
 *     return res.forbiden(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function forbiden(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.forbiden()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.forbiden()` called with an Error:', optionalData);
